...
su propio `CLAUDE.md`** (`backend/`, `frontend/`, `backend/mocks/sap/`) y se cargan al trabajar en esa carpeta.
...
Tres piezas orquestadas con **Docker Compose** (`backend/` y `frontend/` en la raíz; el mock vive
dentro del backend, junto a su consumidor):
...
- **`backend/mocks/sap/`** — Origen de datos simulado. nginx que sirve un `.txt` por HTTP. Host `8000` →
  interno **`8080`**. Imita lo que vendría de SAP; sustituible por SAP real (OData, ficheros, RFC/BAPI).
  Servicio `sap-mock` en compose. → `backend/mocks/sap/CLAUDE.md`
...
- **Nunca commitear datos reales** de SAP: van en `backend/mocks/sap/data-real/` o `.../private/` (ignorados)
  o fuera del repo. Los `.txt` **ficticios** de `backend/mocks/sap/data/` SÍ se commitean (fixtures pedagógicas).
